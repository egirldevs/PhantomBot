...
module.exports.run = async (client, message, args) => {
    ...
    if(!member) {
      return message.channel.send("<:error emoji:> Please mention a user to kick. \n **TIP:** Want to kick someone without mentioning them? \n Copy there id (`Must have developer mode on`) and use that instead mentions.")
    }
    if(!reason) {
      return message.channel.send("<:error emoji:> Please enter a reason to kick this user.")
    } 
    if(!member.kickable) {
        return message.channel.send("<:error emoji:> You cannot ban this user.")
    }
    if(member.id === message.author.id) {
        return message.channel.send("<:error emoji:> You cannot ban yourself.")
    }
    message.author.send(`You kicked ${member} for \`${reason}\``)
