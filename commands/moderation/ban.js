...
module.exports.run = async (client, message, args) => {
  ...
  if(!member) {
    return message.channel.send("<:error emoji:> Please mention a user to ban. \n **TIP:** Want to ban someone without mentioning them? \n Copy there id (`Must have developer mode on`) and use that instead mentions.")
  }
  if(!reason) {
    return message.channel.send("<:error emoji:> Please enter a reason to ban this user.")
  }
        if (member.id === message.author.id) {
        return message.reply("<:error emoji:> You can't ban yourself.")
    }
  if(!member.bannable) {
